...
def process_gif():
    console.print(Panel.fit("[bold blue]Advanced Interactive GIF Compressor[/bold blue] \nPowered by Rich, TQDM, and Pillow"))

    # 1. Get Input File
    input_path = Prompt.ask("[bold green]Enter the path to the input GIF file[/bold green]")
    if not os.path.exists(input_path) or not input_path.lower().endswith('.gif'):
        console.print("[bold red]Error:[/bold red] File does not exist or is not a GIF.")
        sys.exit(1)
    ...
    output_path = Prompt.ask("[bold green]Enter the output path[/bold green]", default=default_output)
    ...
    try:
        ...
        console.print(f"Loaded [bold cyan]{total_frames}[/bold cyan] frames. Starting compression...\n")
        ...
        for frame in tqdm(frames, desc="Compressing Frames", unit="frame", dynamic_ncols=True):
